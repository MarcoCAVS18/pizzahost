...
const PastaSection = () => {
  ...
  const [featuredPasta, setFeaturedPasta] = useState([]);
  const [loading, setLoading] = useState(true);

  // Cargar productos de pasta de Firestore
  useEffect(() => {
    const fetchPastaProducts = async () => {
      try {
        setLoading(true);
        const pastaProducts = await getProductsByCategory('pasta');
        
        // Filtrar para mostrar solo los dos primeros (o específicos si prefieres)
        let featuredItems = pastaProducts;
        if (pastaProducts.length > 2) {
          // Opción 1: mostrar los dos primeros
          featuredItems = pastaProducts.slice(0, 2);
          
          // Opción 2: mostrar productos específicos por ID
          // const pastaNapolitana = pastaProducts.find(p => p.id === 'pasta-napolitana');
          // const pastaCarbonara = pastaProducts.find(p => p.id === 'pasta-carbonara');
          // featuredItems = [pastaNapolitana, pastaCarbonara].filter(Boolean);
        }
        
        setFeaturedPasta(featuredItems);
      } catch (error) {
        console.error("Error loading pasta products:", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-14">
            ...
            <div className="lg:w-1/2 flex flex-col gap-8 justify-center order-last lg:order-first">
              {loading ? (
                <div className="text-center py-12">Loading pasta dishes...</div>
              ) : (
